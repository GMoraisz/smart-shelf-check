...
const Scanner = () => {
  ...
  useEffect(() => {
    ...
    const onScanSuccess = (decodedText: string) => {
      ...
      // Remove o prefixo "qr-" (ignorando maiúsculas/minúsculas)
      let cleanedCode = decodedText;
      if (decodedText.toLowerCase().startsWith('qr-')) {
        cleanedCode = decodedText.substring(3); // Pega a string a partir do terceiro caractere
      }

      setManualCode(cleanedCode); // Atualiza o campo de input com o código limpo
      handleBarcodeSearch(cleanedCode); // Faz a busca com o código limpo
    };
  ...
  const handleBarcodeSearch = async (codeContent: string) => {
    if (!codeContent || !user) {
    ...
    try {
      const { data: productData, error: productError } = await supabase
        .from('products')
        .select('id, name, description, price, stock, image_url, barcode')
        .eq('barcode', codeContent)
        .single();
      ...
      toast({ title: "Produto não encontrado", description: `O código ${codeContent} não corresponde a nenhum produto.`, variant: "destructive" });
    } finally {
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-4">
              <div>
                <Label htmlFor="barcode">Código do Produto</Label>
              ...
              <div className="flex justify-center">
                <Button onClick={() => setIsCameraOpen(!isCameraOpen)} disabled={isSearching} className="w-full max-w-xs" size="lg">
                  ...
                  {isCameraOpen ? 'Fechar Câmara' : 'Escanear Código'}
                </Button>
              </div>
            </div>
          </CardContent>
